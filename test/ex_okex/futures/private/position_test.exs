defmodule ExOkex.Futures.Private.PositionTest do
  use ExUnit.Case
  import TestHelper
  ...
  test "can return crossed positions" do
    ...
    with_mock_request(:get, response, fn ->
      assert {:ok, positions} = Futures.Private.position("BTC-USD-190329", @config)
      assert Enum.count(positions) == 1

      assert [%Futures.CrossedPosition{} = position | _] = positions
      assert position.instrument_id == "BTC-USD-191227"
  ...
  test "can return fixed positions" do
    ...
    with_mock_request(:get, response, fn ->
      ...
      assert [%Futures.FixedPosition{} = position | _] = positions
      assert position.instrument_id == "BTC-USD-191227"
      assert position.long_leverage == "15"
      assert position.short_leverage == "10"
    end)
  end
end
