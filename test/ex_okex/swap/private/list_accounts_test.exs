defmodule ExOkex.Swap.Private.ListAccountsTest do
  use ExUnit.Case
  import TestHelper
  alias ExOkex.Swap
  ...
  test "returns list of accounts" do
    ...
    with_mock_request(:get, response, fn ->
      assert {:ok, [%{"balance" => "0.00"}]} == Swap.Private.list_accounts()
    end)
  end
  ...
  test "accepts dynamically specified config" do
    ...
    with_mock_request(:get, response, fn ->
      assert {:ok, [%{"balance" => "0.00"}]} == Swap.Private.list_accounts(@config)
    end)
  end
end
